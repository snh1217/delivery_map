...
- `NEXT_PUBLIC_KAKAO_JAVASCRIPT_KEY` (또는 `NEXT_PUBLIC_KAKAO_JS_KEY`)가 있으면 앱에서 카카오내비 SDK 상태를 자동 점검합니다.
...
- 운임 항목:
  - 금액(원금) 입력(숫자)
  - 라인별 `로지(-23%)` 체크 가능
  - 체크 시 해당 라인의 실수령 금액이 표시됨
  - `+ 운임 추가`로 라인 추가
  - 합계는 자동 계산되며 **실수령 기준**입니다.
- 합계/저장/통계는 모두 실수령(net) 기준이며 로지 23% 공제가 반영됩니다.
- `오늘 운임 불러오기`:
...
- 앱은 실행 시 카카오내비 사용 가능 여부를 점검하고, 설정 패널에 결과를 표시합니다.
  - JS 키 없음
  - SDK 로드 실패
  - `Kakao.Navi` API 미노출 (도메인 미등록/설정 문제 가능)
