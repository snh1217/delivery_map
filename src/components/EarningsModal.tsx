...
import { formatKRW, MY_EARNING_TARGET_NAME, parseAmount, sumItems } from "@/lib/earnings";
...
type LoadConflictAction = "merge" | "overwrite" | "cancel";
...
function createLine(amount = ""): LocalLine {
  return {
...
function normalizeSavedItemsToLines(items: DailyEarningItem[]): LocalLine[] {
  if (!items.length) {
    return [createLine()];
  }
  return items.map((item) => createLine(String(item.amount || "")));
}
...
export function EarningsModal({ open, onClose }: Props) {
  ...
  const parsedItems = useMemo(
    () =>
      lines
        .map((line) => ({ amount: parseAmount(line.amountText) }))
        .filter((item) => item.amount > 0),
    [lines],
  );
  const totalAmount = useMemo(() => sumItems(parsedItems), [parsedItems]);
  ...
  const applyLoadedRow = (row: DailyEarningRow, action: Exclude<LoadConflictAction, "cancel">) => {
  ...
  const onLoadToday = async () => {
    ...
    try {
      const params = new URLSearchParams({
        target: selectedTarget.targetName,
      });
      if (selectedTarget.targetId) {
        params.set("targetId", selectedTarget.targetId);
      }
  ...
  const updateLine = (lineId: string, rawValue: string) => {
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="max-h-[88vh] overflow-y-auto p-4 sm:max-h-[80vh]">
          ...
          <div className="mt-4 rounded-xl border border-slate-200 p-3">
            ...
            <div className="space-y-2">
              {lines.map((line, index) => (
                <div key={line.id} className="grid grid-cols-[1fr_auto] gap-2">
            ...
            <button
              type="button"
              className="mt-2 h-10 w-full rounded-lg border border-slate-300 bg-white text-sm"
              onClick={addLine}
            >
              + 운임 추가
            </button>
          ...
          <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
            <button
              type="button"
              className="h-11 rounded-lg border border-slate-300 bg-white text-sm disabled:opacity-50"
              onClick={() => void onLoadToday()}
              disabled={loadingToday || saving}
            >
              {loadingToday ? "불러오는 중..." : "오늘 운임 불러오기"}
            </button>
            <button
              type="button"
              className="h-11 rounded-lg bg-cyan-700 text-sm font-medium text-white disabled:opacity-50"
              onClick={() => void onSave()}
              disabled={saving}
            >
              {saving ? "저장 중..." : "저장"}
            ...
            <button
              type="button"
              className="h-11 rounded-lg border border-slate-300 bg-white text-sm"
              onClick={onClose}
              disabled={saving}
            >
              닫기
          ...
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/40 p-4">
            <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-4 shadow-xl">
              ...
              <div className="mt-3 grid gap-2">
                <button
                  type="button"
                  className="h-10 rounded-lg bg-cyan-700 text-sm font-medium text-white"
                  onClick={() => applyLoadedRow(pendingLoadedRow, "merge")}
                >
                  합치기 (기본)
                </button>
                <button
                  ...
                  onClick={() => applyLoadedRow(pendingLoadedRow, "overwrite")}
                >
                  덮어쓰기
                </button>
                <button
