...
export async function GET(request: NextRequest) {
  ...
  try {
    const upstream = `${NAVER_MAPS_API_BASES.directions15}/driving?start=${encodeURIComponent(start)}&goal=${encodeURIComponent(goal)}&option=${encodeURIComponent(option)}`;
    const payload = (await fetchNaverApiJson(upstream)) as Record<string, unknown>;
    return NextResponse.json({ provider: "directions15", raw: payload });
  } catch (error) {
    const message = error instanceof Error ? error.message : "네이버 Directions15 오류";
