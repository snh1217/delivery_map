...
import { getAdminEarningsUserDetail } from "@/lib/earningsStore";
...
export async function GET(request: Request) {
  ...
  try {
    const payload = await getAdminEarningsUserDetail({ phone, from, to });
    return NextResponse.json(payload);
  } catch (error) {
    const message = error instanceof Error ? error.message : "사용자 운임 상세 조회 실패";
